...
export class TournamentsController {
  ...
  async publish(
    ...
    if (!user.roles.includes('ADMIN') && !user.roles.includes('ORGANIZER')) {
      throw new ForbiddenException(
        'Only admins and organizers can publish tournaments',
      );
    }
    return this.publishTournament.execute(
      dto.tournamentId,
      user.id,
  ...
  @ApiOperation({
    summary:
      'Finalize evaluation: lock results, rebuild leaderboard, set finished status',
  })
  async finish(@CurrentUser() user: AuthUser, @Param('id') id: string) {
    if (!user.roles.includes('ADMIN') && !user.roles.includes('ORGANIZER')) {
      throw new ForbiddenException(
        'Only admins and organizers can finalize tournaments',
      );
    }
