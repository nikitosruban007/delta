...
describe('ConsultationsService (unit tests)', () => {
  ...
  describe('startConsultation', () => {
    it('should start consultation if user is host', async () => {
      ...
      mockPrisma.consultation.update.mockResolvedValue(
        mockConsultationRow({ status: 'ACTIVE' }),
      );
    ...
    it('should throw ForbiddenException if user is not host', async () => {
      ...
      await expect(service.startConsultation('c1', 'not-host')).rejects.toThrow(
        ForbiddenException,
      );
    });
    ...
    it('should throw NotFoundException if consultation not found', async () => {
      ...
      await expect(service.startConsultation('c1', 'host-1')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('joinConsultation', () => {
    it('should create participant if not already joined', async () => {
      ...
      mockPrisma.consultationParticipant.create.mockResolvedValue({
        id: 'p1',
        userId: 'user-2',
      });
    ...
    it('should update joinedAt if already joined', async () => {
      ...
      mockPrisma.consultationParticipant.update.mockResolvedValue({
        ...existing,
        joinedAt: new Date(),
      });
    ...
    it('should throw NotFoundException if consultation not found', async () => {
      ...
      await expect(service.joinConsultation('c1', 'user-1')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('leaveConsultation', () => {
    it('should update leftAt for participant', async () => {
      ...
      mockPrisma.consultationParticipant.findFirst.mockResolvedValue(
        participant,
      );
      mockPrisma.consultationParticipant.update.mockResolvedValue({
        ...participant,
  ...
  describe('endConsultation', () => {
    it('should end consultation if user is host', async () => {
      ...
      mockPrisma.consultation.update.mockResolvedValue(
        mockConsultationRow({ status: 'ENDED' }),
      );
    ...
    it('should throw ForbiddenException if not host', async () => {
      ...
      await expect(service.endConsultation('c1', 'not-host')).rejects.toThrow(
        ForbiddenException,
      );
    });
    ...
    it('should throw NotFoundException if not found', async () => {
      ...
      await expect(service.endConsultation('c1', 'host-1')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('edge cases and branch coverage', () => {
    it('should create with all participantIds including createdById (filter self)', async () => {
      ...
      expect(nonHostParticipants.every((p: any) => p.userId !== 'user-1')).toBe(
        true,
      );
    });
